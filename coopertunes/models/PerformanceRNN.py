...
class PerformanceRNN(Model):
    def __init__(self, hparams: PerformanceRNNHParams, device: str = "cuda:0"):
        ...
        self.inithid_fc = nn.Linear(hparams.init_dim, hparams.gru_layers * hparams.hidden_dim)
        ...
        self.output_fc = nn.Linear(hparams.hidden_dim * hparams.gru_layers, self.output_dim)
    ...
    def beam_search(self, init, steps, beam_size, controls=None,
        ...
        if stochastic:
            ...
            beam_log_prob_perturbed = torch.zeros(batch_size, current_beam_size).to(self.device)
